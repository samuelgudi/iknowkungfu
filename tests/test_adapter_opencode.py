...
def src_skill(tmp_path):
    ...
    (src / "SKILL.md").write_text(
        "---\n"
        "name: example\n"
        "description: Use this skill when running the OpenCode adapter test fixture.\n"
        "---\n\n# Example\n\nBody text.\n"
    )
...
def test_detect_finds_user_config_dir(home):
    a = OpenCodeAdapter()
...
def test_target_dir_user_scope_under_config_opencode(home):
    a = OpenCodeAdapter()
    t = a.target_dir("test-author/example", category="dev")
    rel = str(t.relative_to(home)).replace("\\", "/")
    assert rel == ".config/opencode/skills/test-author-example"


def test_target_dir_project_scope_under_dot_opencode(home, monkeypatch, tmp_path):
    proj = tmp_path / "proj"
    proj.mkdir()
    ...
    assert rel == ".opencode/skills/test-author-example"


def test_install_rewrites_name_to_match_folder(home, src_skill):
    """OpenCode requires folder name == frontmatter name. Verify the rewrite
    happens so the canonical install layout is valid for OpenCode."""
    (home / ".config/opencode").mkdir(parents=True)
    a = OpenCodeAdapter()
    ...
    target = home / ".config/opencode/skills/test-author-example"
    assert target.exists()
    installed = (target / "SKILL.md").read_text()
    fm_text = installed.split("---\n", 2)[1]
    fm = yaml.safe_load(fm_text)
    assert fm["name"] == "test-author-example"
    assert target.name == fm["name"], "folder name must match frontmatter name (OpenCode contract)"
...
def test_install_writes_marker(home, src_skill):
    ...
    a.install(src_skill, "test-author/example", "0.3.0", meta=meta,
              opts={"registry_hash": h, "source_url": "x", "tree_sha": "abc"})
    marker_path = home / ".config/opencode/skills/test-author-example" / MARKER_FILENAME
...
def test_uninstall_removes_managed_skill(home, src_skill):
    ...
    assert res.success
    assert not (home / ".config/opencode/skills/test-author-example").exists()
...
def test_list_installed_only_returns_marker_dirs(home, src_skill):
    ...
    meta = json.loads((src_skill / "meta.json").read_text())
    a.install(src_skill, "test-author/example", "0.3.0", meta=meta, opts={"registry_hash": "x"})
    user_authored = home / ".config/opencode/skills/manual"
