...
# ─── Friction #3 (v0.1.7 field test): show resolves install status w/o --agent ─

def test_show_without_agent_finds_install(tmp_path, monkeypatch, capsys):
    """`kfu show <id>` without `--agent` must report the real install status
    by checking detected hosts — not fall back to 'Installed: no'. Friction #3
    from the v0.1.7 Hermes Agent field test."""
    ...
    registry = {
        ...
        "generated_at": "2026-05-11T00:00:00Z",
        "skills": [{
            "id": "test-author/example", "name": "example",
            "description": "test", "version": "0.1.0", "status": "active",
            "author": {"name": "Test", "github_login": "test-author", "github_id": 1},
            "category": "meta", "tags": [], "platforms": ["linux"],
            "agent_compat": ["claude-code"], "license": "MIT", "has_scripts": False,
            "source": {"path": "skills/test-author/example",
                       "content_hash": h, "files": ["SKILL.md", "meta.json"]},
        }],
    }
    (cache / "registry.json").write_text(json.dumps(registry))
